...
s_minY = Slider(ax_minY, 'Y', np.log10(minyRange[0]), np.log10(minyRange[2]), valinit=np.log10(minyRange[1]), valfmt="%0.4f")
s_minY.valtext.set_text( pow(10., s_minY.val))

ax_midY = plt.axes([ledge+wdt+hspc, 0.1, wdt, hgt], facecolor='white')
s_midY = Slider(ax_midY, 'Y', np.log10(midyRange[0]), np.log10(midyRange[2]), valinit=np.log10(midyRange[1]), valfmt="%0.1f")
s_midY.valtext.set_text( pow(10., s_midY.val))

ax_maxY = plt.axes([ledge+2*wdt+2*hspc, 0.1, wdt, hgt], facecolor='white')
s_maxY = Slider(ax_maxY, 'Y', np.log10(maxyRange[0]), np.log10(maxyRange[2]), valinit=np.log10(maxyRange[1]), valfmt="%i")
